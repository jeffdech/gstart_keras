...
class NaiveDense:
    def __init__(self, input_size, output_size, activation):
        ...
        w_init_val = tf.random.uniform(w_shape, minval=0, maxval=0.1)
...
def one_training_step(model, images_batch, labels_batch):
    with tf.GradientTape() as tape:
        predicitons = model(images_batch)
        per_sample_loss = tf.keras.losses.sparse_categorical_crossentropy(labels_batch, predicitons)
